...
// Added an imageFile property to hold the actual File object
const form = reactive({
  title: "",
  type: "",
  eventTime: "", // use datetime-local value directly
  status: "",
  studentId: "",
  description: "",
  response: "",
...
// Capture the file when the user selects an image
...
const openModal = (event = null) => {
  if (event) {
    ...
    if (event.eventTime) {
      ...
      form.eventTime = d.toISOString().slice(0, 16);
    }

    let desc = event.description || "";
    let loc = "";
    const locationMatch = desc.match(/^\[Location:\s*(.*?)\]\s*(.*)/s);
    if (locationMatch) {
      loc = locationMatch[1];
      desc = locationMatch[2].trim();
    }
    ...
    Object.assign(form, {
      ...
      location: loc,
      description: desc,
      imageFile: null, // Don't require a new image if they are just editing text
    });
  } else {
    resetForm();
  }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // These 8 keys MUST perfectly match the error log you just showed me
    ...
    formData.append("response", form.response || "N/A");

    if (form.imageFile) {
      // Changed from 'files' to 'fileUrl' based on the API error!
      formData.append("fileUrl", form.imageFile);
    }

    if (isEditing.value) {
      await eventsStore.updateEvent(editingId.value, formData);
    } else {
